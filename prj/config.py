...
class DetectionConfig:
    ...
    use_dlib: bool = True
    calibration_period: int = 50 # Adjusted for faster calibration


@dataclass
class DrowsinessConfig:
    """Drowsiness detection thresholds."""
    ear_threshold_drowsy: float = 0.22
    ear_threshold_critical: float = 0.18
    mar_threshold_yawn: float = 0.65
    head_tilt_threshold: float = 15.0  # Added parameter
    consecutive_frames_drowsy: int = 15
    consecutive_frames_critical: int = 30
    feature_history_length: int = 30
    calibration_period: int = 50  # Added here for metrics.py
...
class AppConfig:
    ...
    show_fps: bool = True
    show_landmarks: bool = True  # Show facial landmarks
